...
侍从修炼 (youth) → knight_examination (teenager) → 骑士锦标赛/城堡围攻/屠龙 (youth/adult)
                                       ↓
                               传说的传播 (elder: req chr≥15 + dragon_slayer + knight)

加入冒险者公会 (teenager) → 猎人公会任务/高价悬赏/未踏之地 (youth/adult)
                          → 工会晋升 (adult)

魔法学院来信 (youth) → magic_exam (youth) → 骑士的荣光/学院决斗赛 (teenager)
                          → magic_theory_class (youth) → 最后的魔法 (elder: req mag≥22)

感情升温 (youth) → 求婚 (adult) → 婚礼 (adult) → 结婚周年纪念 (adult)
                                        → family_blessing/初为人父母 (adult)
                                        → family_dinner (adult)
                                        → human_grandchildren (middle-age)
                                        → 含饴弄孙/给孩子讲故事/全家聚会 (elder)

收徒传艺 (adult) → 传承之问 (middle-age: req has_student + married + lord)
                → 培养继承人 (middle-age)
                → 故徒来访 (elder)
```
...
|----------|---------------|-------------|
| 没有 `engaged` flag | 「求婚」「婚礼」 | `marriage_proposal`, `wedding_ceremony` |
| 没有 `married` flag | 「结婚周年纪念」「家庭晚餐」 | `marriage_anniversary`, `family_dinner` |
| 没有 `parent` flag | 「含饴弄孙」「给孩子讲故事」 | `human_grandchildren`, `human_grandchild_story` |
| 没有 `knight` flag | 「骑士锦标赛」「城堡围攻」 | `knight_tournament`, `knight_siege` |
| 没有 `guild_member` flag | 「猎人公会任务」「高价悬赏」 | `monster_hunt_guild`, `adv_bounty` |
| 没有 `magic_student` flag | 「魔法考试」「魔法理论课」 | `magic_exam`, `magic_theory_class` |
| 没有 `squire` flag | 「骑士考核」 | `knight_examination` |
| 没有 `has_student` flag | 「传承之问」「培养继承人」 | `legacy_question`, `mid_heir_training` |
| 没有 `dragon_slayer` flag | 「传说的传播」 | `legend_spread` |
...
| 超寿命惩罚 | `lifeProgress > 1.0` 时额外衰减 `floor((lifeProgress-1.0) × 60)` | 超过 maxLifespan 的角色应快速死亡 |
...
| `dragon_slay_attempt` 讨伐巨龙 | `str ≥ 14 & mag ≥ 14` | 体魄 5 的角色不应触发 |
| `marry_noble` 政治联姻 | `chr ≥ 10 & mny ≥ 8 & (knight \| guild)` | 魅力 3 无 flag 不应触发 |
| `marriage_proposal` 求婚 | `has.flag.engaged` | 没有 engaged flag 不应触发 |
| `adult_guild_promotion` 工会晋升 | `has.flag.guild_member`（+ 属性） | 无 flag 不应触发 |
| `legend_spread` 传说的传播 | `chr ≥ 15 & dragon_slayer & knight` | 三条件缺一不可 |
| `peaceful_end` 平静的终章 | `spr ≥ 8 & has.flag.parent` | 无 parent flag 不应触发 |
| `magic_breakthrough_final` 最后的魔法 | `mag ≥ 22` | 魔力 10 不应触发 |
| `legacy_question` 传承之问 | `has_student & married & lord` | 三 flag 缺一不可 |
