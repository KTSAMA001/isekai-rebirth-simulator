...
#### 合理预期（供参考，可根据实际模拟结果调整）

| 种族 | 寿命范围 | maxLifespan | 预期平均存活年龄 |
|------|---------|-------------|----------------|
| 人类 | 40-70 | 100 | 55-75 |
| 精灵 | 200-400 | 500 | 250-400 |
| 矮人 | 150-300 | 450 | 180-300 |
| 哥布林 | 20-35 | 60 | 22-38 |

#### 判定规则

1. 平均存活年龄应在寿命范围（lifespanRange）的 50%-95% 之间
2. 中位数不应偏离平均超过 20%
3. 不应出现存活年龄 > maxLifespan 的情况
...
_最后更新：2026-04-15（基于 Phase 0-2 + 初始数值恢复后的代码状态：默认属性 0，分配点数 20，允许负数出生）_
