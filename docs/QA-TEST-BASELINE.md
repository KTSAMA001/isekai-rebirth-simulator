...
| adult | [20, 45] | [80, 250] | [15, 40] | [60, 220] |
| midlife | [35, 60] | [200, 380] | [30, 50] | [180, 320] |
| elder | [55, 100] | [350, 500] | [40, 60] | [280, 400] |
...
- **人类预期死亡 ~62 岁**：Beta(8,3) 均值 0.727 × 100（maxLifespan）≈ 73，实际平均约 71 岁
- **哥布林预期死亡 ~45 岁**：Beta(8,3) 均值 0.727 × 60 ≈ 44，lifespanRange [25, 45]
...
- 下限 0.60 → 最早在 60% 寿命时开始衰老（人类 60 岁、精灵 300 岁、哥布林 36 岁）
- 上限 0.92 → 最晚在 92% 寿命时开始衰老（人类 92 岁、精灵 460 岁、哥布林 55 岁）

注意：personalDeathProgress 是衰老开始点，不是死亡点。
实际死亡进度（HP归零时的 lifeProgress）更高，因为有 HP 恢复和衰减上限的缓冲。
```

#### 纯衰减死亡分布（蒙特卡洛 5000 次，str=5，无事件伤害）

| 种族 | maxLifespan | 平均 | 中位数 | P25 | P75 | 最小 | 最大 | Range上界超限率 |
|------|-------------|------|--------|-----|-----|------|------|----------------|
| 人类 | 100 | 70.6 | 71.0 | 62 | 78 | 57 | 88 | 7.0% |
| 精灵 | 500 | 314.5 | 317.0 | 269 | 355 | 244 | 401 | 3.9% |
| 哥布林 | 60 | 44.7 | 45.0 | 39 | 49 | 37 | 55 | — (新Range) |
| 矮人 | 400 | 254.4 | 256.0 | 218 | 287 | 198 | 324 | — (新Range) |

**关键发现**：lifespanRange 上界应覆盖 P75 左右（约 75% 的角色在此前死亡），下界应覆盖 P25 左右。
据此调整了哥布林和矮人的 lifespanRange（见第 2 节）。
...
### A. 各种族寿命分布
| 种族 | maxLifespan | lifespanRange | 预期大部分死亡区间 | 关注点 |
|------|-------------|---------------|-------------------|--------|
| 人类 | 100 | [65, 85] | 60-80 岁 | 少数早亡/长寿 |
| 精灵 | 500 | [250, 400] | 260-360 岁 | elder 事件 350+ |
| 哥布林 | 60 | [25, 45] | 37-50 岁 | elder 事件 40+ |
| 矮人 | 400 | [180, 290] | 210-300 岁 | elder 事件 280+ |
...
用蒙特卡洛模拟（50-100 次），统计各种族的存活年龄分布。

#### 实际分布（纯衰减 5000 次，str=5，无事件伤害）

| 种族 | lifespanRange | maxLifespan | 平均 | 中位数 | P25-P75 | 超Range上界 |
|------|--------------|-------------|------|--------|---------|------------|
| 人类 | [65, 85] | 100 | 70.6 | 71.0 | 62-78 | 7.0% |
| 精灵 | [250, 400] | 500 | 314.5 | 317.0 | 269-355 | 3.9% |
| 哥布林 | [25, 45] | 60 | 44.7 | 45.0 | 39-49 | ~0% |
| 矮人 | [180, 290] | 400 | 254.4 | 256.0 | 218-287 | ~0% |

注：实际游戏中有事件伤害，死亡年龄会比纯衰减更早、分布更广。

#### 判定规则

1. 平均存活年龄应在 lifespanRange 的 70%-110% 之间
2. 中位数不应偏离平均超过 20%
3. 不应出现存活年龄 > maxLifespan 的情况（允许极少数由于超寿命惩罚延迟）

#### 自动化测试

- `tests/qa-baseline-verify.test.ts` 第 9.9 节：使用引擎 API 跑 50 次模拟/种族，验证上述判定规则

---

_最后更新：2026-04-15（存活年龄分布调查：调整哥布林/矮人 lifespanRange 和 lifeStages，基于 5000 次蒙特卡洛模拟）_
