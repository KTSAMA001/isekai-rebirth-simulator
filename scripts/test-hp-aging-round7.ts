/**
 * 第七轮 HP 衰老系统验证
 * 折中方案：短寿命补偿系数 3.5 + 随机健康波动降20%
 * 每个种族 20 轮，共 80 轮
 *
 * 运行: npx tsx scripts/test-hp-aging-round7.ts
 */
...
async function main() {
  ...
  console.log('第七轮 HP 衰老系统验证 — 折中方案')
  console.log('═'.repeat(80))
  console.log()
  console.log(`配置: 每种族 ${RUNS_PER_RACE} 轮，共 ${RACES.length * RUNS_PER_RACE} 轮`)
  console.log(`短寿命补偿系数: 3.5`)
  console.log('  哥布林: +2/年 (maxAge≈33, 3.5×0.835≈2.92→2)')
  console.log('  人类:   +2/年 (maxAge≈85, 3.5×0.575≈2.01→2)')
  console.log('  矮人:   +1/年 (maxAge≈170, 3.5×0.15≈0.53→1)')
  console.log('  精灵:   +0/年 (maxAge≈400, 3.5×(-1.0)=0→0)')
  console.log(`随机健康波动: 概率和伤害量各降20%`)
  console.log(`  概率: (0.08 + lifeRatio*0.12) × 0.8`)
  console.log(`  伤害: round(baseDmg × 0.8)`)
  console.log()
  ...
  for (const race of RACES) {
    ...
    const verdict = pass ? '✅ 通过' : normalPct < 70 ? '⚠️ 偏低' : '⚠️ 偏高'
